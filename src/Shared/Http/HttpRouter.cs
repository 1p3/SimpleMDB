...
public class HttpRouter
{
    ...
    {
        Func<Task> globalMiddlewarePipeline = GenerateMiddlewarePipeline(
            req,
            res,
            props,
            middlewares
    ...
    private async Task SimpleRouteMatching(
        HttpListenerRequest req,
        HttpListenerResponse res,
        Hashtable props,
        Func<Task> next
    )
    {
        foreach (var (method, path, middlewares) in routes)
        {
            ...
            {
                Func<Task> routeMiddlewarePipeline = GenerateMiddlewarePipeline(
                    req,
                    res,
                    props,
                    middlewares.ToList()
    ...
    private async Task ParametrizedRouteMatching(
        HttpListenerRequest req,
        HttpListenerResponse res,
        Hashtable props,
        Func<Task> next
