...
public static class HttpUtils
{
    ...
    {
        var requestId =
            props["req.id"]?.ToString() ?? Guid.NewGuid().ToString("n").Substring(0, 12);
    ...
    public static NameValueCollection ParseUrl(string url)
    {
        int i = -1;
        var (scheme, apqf) =
            (i = url.IndexOf("://")) >= 0 ? (url.Substring(0, i), url.Substring(i + 3)) : ("", url);
        var (auth, pqf) =
            (i = apqf.IndexOf("/")) >= 0 ? (apqf.Substring(0, i), apqf.Substring(i)) : (apqf, "");
        var (up, hp) =
            (i = auth.IndexOf("@")) >= 0
                ? (auth.Substring(0, i), auth.Substring(i + 1))
                : ("", auth);
        var (user, pass) =
            (i = up.IndexOf(":")) >= 0 ? (up.Substring(0, i), up.Substring(i + 1)) : (up, "");
        var (host, port) =
            (i = hp.IndexOf(":")) >= 0 ? (hp.Substring(0, i), hp.Substring(i + 1)) : (hp, "");
        var (pq, fragment) =
            (i = pqf.IndexOf("#")) >= 0 ? (pqf.Substring(0, i), pqf.Substring(i + 1)) : (pqf, "");
        var (path, query) =
            (i = pq.IndexOf("?")) >= 0 ? (pq.Substring(0, i), pq.Substring(i + 1)) : (pq, "");
    ...
    public static string DetectContentType(string text)
    {
        ...
        if (s.StartsWith("{") || s.StartsWith("["))
        {
            ...
            s.StartsWith("<!DOCTYPE html", StringComparison.OrdinalIgnoreCase)
    ...
    public static async Task SendNotFoundResponse(
        HttpListenerRequest req,
        HttpListenerResponse res,
        Hashtable props
    )
    ...
    )
    {
        ...
        if (prev != null)
        {
            linkParts.Add($"<{prev}>; rel=\"prev\"");
        }
        if (next != null)
        {
            linkParts.Add($"<{next}>; rel=\"next\"");
        }
        if (first != null)
        {
            linkParts.Add($"<{first}>; rel=\"first\"");
        }
        if (last != null)
        {
            linkParts.Add($"<{last}>; rel=\"last\"");
        }
